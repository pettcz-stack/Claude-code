...
2. **Tento projekt** – na GitHubu repo `pettcz-stack/claude-code`, větev
   ...
   rozbal třeba do `C:\workview`.
3. **WorkViewAgent.msi** – z GitHub → Actions → „Monitoring Agent (Windows build)"
...
1. Soubor `monitoring\installer\install-agent-test.bat` **zkopíruj do složky,
   kde máš `WorkViewAgent.msi`** (aby byly vedle sebe).
...
- Agent: pošli soubor `%TEMP%\WorkViewAgent-install.log` a `C:\ProgramData\WorkView\agent.log`.
