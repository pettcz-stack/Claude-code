...
import { computeTopInsights } from './riskSignals.js';
...
export const cq = {
  ...
  insights: (from: string, to: string, dept?: Dept) =>
    memo(`insights:${d(from)}:${d(to)}:${k(dept)}`, () => computeTopInsights(new Date(from), new Date(to), dept)),
