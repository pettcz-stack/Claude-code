...
import { startOfUtcDay, minutesToHm, utcHourOf } from './util.js';
...
export function CalendarView({ user, day }: Props) {
  ...
  for (const r of rows) byHour.set(utcHourOf(r.hourStart), r);
  ...
  return (
    <div>
            ...
            {Array.from({ length: 24 }, (_, h) => {
              ...
              return (
                <tr key={h} className="border-t border-gray-100">
                  ...
                  <td className="px-3 py-2 text-gray-700">{r?.topApp ?? <span className="text-gray-300">—</span>}</td>
      ...
      <div className="mt-3 flex gap-4 text-xs text-gray-500">
        ...
        <span className="ml-auto">Časy v UTC (lokalizace časové zóny v dalším bloku)</span>
      </div>
    </div>
  );
}
